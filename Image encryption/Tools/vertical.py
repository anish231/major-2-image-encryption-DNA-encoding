    ...
    for pixel_coordinate_of_y in range(0, height):
        for pixel_coordinate_of_x in range(0, width):
            ...
            value = pixels[pixel_coordinate_of_x, pixel_coordinate_of_y][color_index_of_rgb] * \
                pixels[pixel_coordinate_of_x, pixel_coordinate_of_y +
                       1][color_index_of_rgb]+value
...
def co2(color_index_of_rgb, height, width, pixels):
    ...
    for pixel_coordinate_of_y in range(0, height):
        for pixel_coordinate_of_x in range(0, width):
            ...
            value_of_y = pixels[pixel_coordinate_of_x,
                                pixel_coordinate_of_y+1][color_index_of_rgb]+value_of_y
    ...
    for pixel_coordinate_of_y in range(0, height):
        for pixel_coordinate_of_x in range(0, width):
            if pixel_coordinate_of_y+1 == height:
                break
            value = (pixels[pixel_coordinate_of_x,
                     pixel_coordinate_of_y+1][color_index_of_rgb]**2)+value

    xy = (value*height*width)-(value_of_y**2)
    return xy


def corr_of_rgb_vertical(loc):
    global value_of_y
    global value_of_x
    ...
    b = ((co1(2, height, width, pixels) - co2(2, height, width, pixels)) /
         (co3(2, height, width, pixels) * co4(2, height, width, pixels)))
